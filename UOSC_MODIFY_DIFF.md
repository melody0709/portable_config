...
| 版本   | 日期       | 修改内容                                                     |
| ------ | ---------- | ------------------------------------------------------------ |
| V1.6.4 | 2026-03-24 | 合并 `path` 双监听为单监听；回看续播改为纯 `end-file` 驱动并移除轮询 timer/续播 seek；续播起点改为 `start_utc + duration`（含 `duration` 缓存兜底）；增强回看 URL 的 `starttime/endtime` 与 `ztestarttime/zteendtime` 参数重写；新增续播调试日志 |
